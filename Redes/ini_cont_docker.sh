...
if ! docker images | grep 'prac3' ; then
    ...
    if ! docker network ls | grep 'INTRANET'; then
    ...
    for contenedor in "${contenedores[@]}"; do
        if ! docker container ls -a | grep "$contenedor"; then
            if [ "$contenedor" == 'Router' ]; then
                docker create -itd --rm --cap-add=NET_ADMIN --cap-add=SYS_ADMIN --hostname "${contenedoresAux[$i]}" --network=ISP --name "$contenedor" "$imatge"
                docker network connect DMZ "$contenedor"
                docker network connect INTRANET "$contenedor"
            else
                docker create $OPCIONS --hostname "${contenedoresAux[$i]}" --network=ISP --name "$contenedor" "$imatge"
